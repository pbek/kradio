...
bool OSSSoundDevice::resumePlayback(SoundStreamID /*id*/)
{
    return false;
}
...
//         logWarning(i18n("%1: Playback buffer overflow. Skipped %1 bytes", m_DSPDeviceName, QString::number(m_PlaybackSkipCount)));
//         m_PlaybackSkipCount = 0;
//     }
...
void OSSSoundDevice::slotPoll()
{
    ...
    if (m_CaptureStreamID.isValid() && m_DSP_fd >= 0) {
        ...
        if (bytesRead > 0) {
            ...
            err = -1;
            logError(i18n("OSS device %1: No data to record", m_DSPDeviceName));
        } else {
            err = errno;
        }
        ...
        while (m_CaptureBuffer.getFillSize() > m_CaptureBuffer.getSize() / 3) {
            ...
            notifySoundStreamData(m_CaptureStreamID, m_DSPFormat, buffer, size, consumed_size, SoundMetaData(m_CapturePos, cur_time - m_CaptureStartTime, cur_time, i18n("internal stream, not stored (%1)", m_DSPDeviceName)));
...
bool OSSSoundDevice::openDSPDevice(const SoundFormat &format, bool reopen)
{
    ...
    bool err = m_DSP_fd < 0;
    if (err) {
        logError(i18n("Cannot open DSP device %1", m_DSPDeviceName));
        return false;
    }
    int caps = 0;
    err |= (ioctl (m_DSP_fd, SNDCTL_DSP_GETCAPS, &caps) != 0);
    if (err)
        logError(i18n("Cannot read DSP capabilities for %1", m_DSPDeviceName));
    ...
    if (err)
        logError(i18n("Cannot set stereo mode for %1", m_DSPDeviceName));
    ...
    if (err || sampleSize != m_DSPFormat.m_SampleBits)
        logError(i18n("Cannot set sample size for %1", m_DSPDeviceName));
    ...
    if (err)
        logError(i18n("Cannot set buffers for %1", m_DSPDeviceName));

    int bufferBlockSize = 0;
    err |= ioctl (m_DSP_fd, SNDCTL_DSP_GETBLKSIZE, &bufferBlockSize);
    if (err) {
        logError(i18n("Cannot read buffer size for %1", m_DSPDeviceName));
    } else {
        logInfo(i18n("%1 uses buffer blocks of %2 bytes", m_DSPDeviceName, QString::number(bufferBlockSize)));
        size_t  tmp = (((m_BufferSize - 1) / bufferBlockSize) + 1) * bufferBlockSize;
        setBufferSize(tmp);
        logInfo(i18n("adjusted own buffer size to %1 bytes", QString::number(tmp)));
    }
...
bool OSSSoundDevice::openMixerDevice(bool reopen)
{
    ...
    if (m_Mixer_fd < 0) {
        logError(i18n("Cannot open mixer device %1", m_MixerDeviceName));
    } else {
        m_PollingTimer.start(40);
    }
    return m_Mixer_fd >= 0;
...
void OSSSoundDevice::getMixerChannels(int query, QStringList &retval, QMap<QString, int> &revmap) const
{
    ...
    if (fd < 0) {
        logError(i18n("OSSSoundDevice::getMixerChannels: Cannot open mixer device %1", m_MixerDeviceName));
    }
    ...
    if (fd >= 0) {
        ...
        if ( ioctl(fd, MIXER_READ(query), &mask) == 0 ) {
            ...
            logError(i18n("OSSSoundDevice::getMixerChannels: Cannot read mixer device mask on device %1", m_MixerDeviceName));
        }
    }
    if (fd != m_Mixer_fd)
        close(fd);
...
{
    return i18n("OSS Sound Device %1", PluginBase::name());
}
